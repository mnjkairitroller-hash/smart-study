...
export default function ProfileView() {
  ...
  const getTimelineLessons = () => {
    ...
    // For each subject, flat-map all video parts in chronological sequence
    const availableQueues: Record<string, any[]> = {};
    sortedSubjects.forEach(subject => {
      ...
      availableQueues[subject] = queue.filter(v => !completed.includes(v.id));
    });
    ...
    const availableForTomorrow: Record<string, any[]> = {};
    sortedSubjects.forEach(subject => {
        availableForTomorrow[subject] = availableQueues[subject].filter(v => !todayLessons.some(t => t.id === v.id));
    });
    ...
    sortedSubjects.forEach(subject => {
      const isEnglish = (subject?.toLowerCase() || '').includes('eng');
      if (isEnglish && !tomorrowEnglishDay) return;
    ...
    // Day After available videos
    const availableForDayAfter: Record<string, any[]> = {};
    sortedSubjects.forEach(subject => {
        availableForDayAfter[subject] = availableForTomorrow[subject].filter(v => !tomorrowLessons.some(t => t.id === v.id));
    });
    ...
    sortedSubjects.forEach(subject => {
      const isEnglish = (subject?.toLowerCase() || '').includes('eng');
      if (isEnglish && !dayAfterEnglishDay) return;
