  ...
  const generatePersonalizedQuiz = async () => {
    ...
    try {
      ...
      if (selectedChapters.length === 0) {
        if (activeRoutineVideos && activeRoutineVideos.length > 0) {
          const routineChapterIds = activeRoutineVideos.map((v: any) => v.chapterId);
          selectedChapters = chapters.filter((ch: any) => routineChapterIds.includes(ch.id));
        }
      }
